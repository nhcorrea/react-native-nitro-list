...
// ±1× split, redistributed toward where the user is heading (LegendList's
// numbers, which it could not enable safely from the JS thread).
...
void LayoutCore::applyTypeMeansLocked() {
  if (!typeAverages_ || typeStats_.empty()) {
    return;
  }
