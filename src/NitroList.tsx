...
function NitroListInner<T>(props: NitroListProps<T>, ref: React.Ref<NitroListHandle>) {
  ...
  const waitForLayoutSettle = useCallback(
    async (trace?: string[]) => {
      ...
      if (commitCounterRef.current < pendingCommitRef.current) {
        ...
        const by = await Promise.race([
          new Promise<'evt'>((resolve) => commitWaitersRef.current.push(() => resolve('evt'))),
      ...
      const awaiting = cellBridgeRef.current.awaitingLayout;
      if (awaiting > 0) {
        const t = trace ? Date.now() : 0;
        const by = await Promise.race([
          new Promise<'evt'>((resolve) => layoutSettleWaitersRef.current.push(() => resolve('evt'))),
          waitForLayoutPass().then(() => 'raf' as const),
        ]);
      ...
      const pendingSizes = pendingSizesRef.current.count;
      if (pendingSizes > 0) flushPendingItemSizes();
      ...
      if (animated) {
        ...
        try {
          ...
          for (let step = 0; step < SCROLL_TO_INDEX_STEPS; step++) {
            ...
            await waitForLayoutSettle(devTrace);
            if (commandId !== scrollCommandIdRef.current) return;
            devMark(`step${step}`);
